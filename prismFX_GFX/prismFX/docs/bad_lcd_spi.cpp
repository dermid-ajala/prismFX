...
void LCD_SPI::drawCharCustom(uint8_t c){
	...
	if(c < font3.first){			// newline or draw a printable character or do nothing
		if(	c != '\n')	return;		// character code is out of range
	}else	c -= font3.first;

/*
	How to set up a custom font.
	1. Any GFX font header file can be used. Copy the file into the plugin\device folder
	2. Rename the header file font3.h
	3. Edit the font3.h file
		A. delete all PROGMEM references
		B. change the #include <Adafruit_GFX.h> line to #include "gfxfont.h"
		C. Near the end of the file, change "const GFXfont FreeMonoBold12pt7b PROGMEM = {" to
											"const GFXfont font3 = {"
	4. Back color does not work well with 


*/
	if(foreC == backC){		// TRANSPARENT PRINTING - MOST SIMILAR TO GFX PRINTING BUT SLOOOOW

		if(c == '\n'){					// advance to start of next line - transparent mode
			rowC += font3.yAdvance;
			colC = 0;
			return;
		}								
		...
		for(int i=0; i<h; i++){			// scan the bits in the bitmap table
			for(int j=0; j<w; j++){
				if(*p & mask)			// if bit set, set pixel
					drawPixel(colC+j+font3.glyph[c].xOffset, rowC+i+font3.glyph[c].yOffset, foreC);
	...
	}else{					// NOT TRANSPARENT - FASTER BUT may be messy with backcolor

		if(c == '\n'){						// clear  to end of line
			for(iint i=1; i<font3.yAdvance; i++)
		...
		int16_t x, y, h = font3.glyph[c].height, w = font3.glyph[c].width;
		uint16_t bufr[w*h], *q = bufr;
		uint8_t mask = 0x80, *p = font3.bitmap + font3.glyph[c].bitmapOffset;

		while(q < bufr + w * h){				// scan the bits in the bitmap table
			*q++ = *p & mask ? foreC : backC;	// if bit set, set pixel
			mask >>= 1;							// ready to test next bit
			if(mask == 0){						// if done with this byte
				mask = 0x80;					// prepare to test the next byte
...
void LCD_SPI::clear(uint8_t rotation, uint32_t color) {
	...
	rowC = font3.yAdvance;								// for GFX characters
}
