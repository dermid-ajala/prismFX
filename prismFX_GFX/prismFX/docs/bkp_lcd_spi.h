...
typedef struct pData{
	float data[240];
	float min, max;
	char units[8], variable[8], format[8];
	uint16_t color;
} plotData;
...
class LCD_SPI : public Device {
	...
	private:
// properties
		enum { cmd_clear, cmd_print, cmd_printTiny, cmd_setColor, cmd_point, cmd_line, cmd_rect, cmd_circle, cmd_plot} cmd_code;
...
// methods (functions)
		...
		void drawPlotDetail(uint16_t pnt, uint8_t arg, bool redraw);
	...
	public:
		...
		void print      (uint8_t row, uint8_t col, char *message, uint8_t siz);
		...
		void initPlot	(uint8_t index, char *variable, char * units, char decimals);
		void plotPoint	(double r, double g, double b);
};
