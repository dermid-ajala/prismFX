	...
	for(uint16_t y=0; y<h && y<16; y++){			// scan the bits in the bitmap table, max 16
		for(uint16_t x=0; x<w; x++){				// for the width
			...
			if(bit)									// if bit is set in the GFX bitmap, set the bit in the output buffer
				gfx_buffer[bufLen + x + fnt->glyph[c].xOffset] |= (1<<(fnt->glyph[c].yOffset + y + 1 + fnt->yAdvance/2));
		}
	}
...
void PrismFX::printGFX(uint8_t col, uint8_t row, char *message){
	...
	fontHeight = 0;	// depends on font sizes and whether TH or EN or both characters are used
	...
	while(*p  && bufLen+32 < 240)	// stop if terminal null character or bitmap buffer near capacity
		if(*p == 0xe0 && (p[1] == 0xb8 || p[1] == 0xb9)){	// is it a Thai character?
			if(thFont.yAdvance > fontHeight)	fontHeight = thFont.yAdvance;
			...
			if(enFont.yAdvance > fontHeight)	fontHeight = enFont.yAdvance;
	...
	printf("fontHeight %d, message width %d\n", fontHeight, bufLen);// help visualizing during development - printf code can be removed
	...
	if(foreC == backC){										// transparent printing
		for(int x=0; x+col<240 && x<bufLen; x++)
			for(int y=0; y+row<240 && y<fontHeight; y++)
				if(gfx_buffer[x] & 1<<y)
					drawPixel(x+col, y+row, foreC);
	}else{													// foreground/background printing
		uint8_t buf[fontHeight][2];
		for(int pos=0; pos<bufLen && col<240; pos++, col++){
			memset(buf, 0, sizeof(buf));
			for(int bit=0; bit<fontHeight; bit++){
				buf[bit][0] = gfx_buffer[pos] & 1<<bit ? foreC >>  8 : backC >>  8;
				buf[bit][1] = gfx_buffer[pos] & 1<<bit ? foreC & 255 : backC & 255;
			}
			setAddrWindow(col, row, col, row + fontHeight - 1);
			bufferOut( (uint8_t *)buf, sizeof(buf) );
		}
	}
}
